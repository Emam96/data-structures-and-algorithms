...
// Solution code here...
  ...
  if (arr.length > 0) {
    arr.forEach((ele, idx) => {
      if (ele.length > elmLength) {
        elmLength = ele.length;
        theLongest = idx;
      }
    });
  }
...
const findHappiness = (arr) => {
  ...
  arr.map(x => {
    ...
    (x.includes(':)')) ? newArr.push(x) : false;
...
const standardizePhoneNumbers = (arr) => {
  ...
  // condition ? exprIfTrue : exprIfFalse
  return arr.map( x => {
    return x.substring(1, 4) + x.substring(6, 9) + x.substring(10, 14);
  });
...
const allHappy = (arr) => {
  ...
  arr.forEach(x => {
    ...
    (x.includes(':)')) ? newArr.push(b) : newArr.push(a);
