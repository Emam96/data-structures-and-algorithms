...
function transformToLis(obj) {
  // Solution code here...
  let valO = Object.entries(obj);

  let newArr = [];

  valO.forEach((item) => {
    let okayA = '<li>' + item[0] + ': ' + item[1] + '</li>';
    newArr.push(okayA);
  });
...
let findShortest = (data) => {
  ...
  let target;
  let shortestHeight = 175;
  data.forEach(ele => {

    if (parseInt(ele.height) < shortestHeight) {
      target = ele.name;
      shortestHeight = parseInt(ele.height);
