...
function addExclamation(arr) {
  ...
  let added = [];
  arr.forEach( (stringy)=>{
    let newStringy =  stringy +'!';
    added.push(newStringy);
  });
...
const allUpperCase = (arr) => {
  ...
  let upper = [];
  arr.forEach( (stringy)=>{
    let upperString = stringy.toUpperCase();
    upper.push(upperString);
  });
...
const createList = (availableItems) => {
  ...
  let upper = [];
  availableItems.forEach( (stringy)=>{
    if (stringy.available === true) {
  
      upper.push(stringy.name);
    } 
  });
