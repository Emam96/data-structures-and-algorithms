...
const screenForNames = (arr) => {
  // Solution code here...
  let regexThing = /^((Mr||Mrs||Dr||Ms).\s)\w+/g;
  return arr.filter((item) => {
    return regexThing.test(item) ? item : false;
  });
};
...
const toTitleCase = (arr) => {
  ...
  for (var i = 0; i < arr.length; i++) {
    let newStuff = arr[i].charAt(0).toUpperCase() + arr[i].slice(1);

    newArr.push(newStuff);
  }
...
let biggerThanLuke = (arr) => {
  ...
  let newArr = [];
  arr.map((x) => {
    x.mass > massScale ? newArr.push(x.name) : false;
  });
...
const isSecure = (url) => {
  ...
  return url.slice(0, 8).includes("https://");
};
...
const detectTicTacToeWin = (board) => {
  ...
  // let winner = board.map((x) => {
  //   // return x[0] === x[1] && x[1] === x[2] && x[0] === x[2] ? true : false;
  //   for (let index = 0; index < x.length; index++) {
  //     return x[index][0] === x[index][0] && x[index][1] === x[index][2] && x[index][0] === x[index][2] ? true : false;
  //   }
  // });
